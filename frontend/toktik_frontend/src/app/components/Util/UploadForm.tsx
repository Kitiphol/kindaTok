  ...
  //       const res = await fetch('http://localhost:8090/videos', {
  //         method: 'POST',
  //         headers: {
  ...
  //       // 2. Upload directly to R2 via a “simple” PUT (no custom headers → no OPTIONS preflight)
  //       const uploadResponse = await fetch(presignedURL, {
  //         method: 'PUT',
  //         // wrap file in a Blob with blank type so browser won’t set Content-Type
  //         body: new Blob([videoFile], { type: '' }),
  //       });
...
//   const [title, setTitle] = useState('');
//   const [uploading, setUploading] = useState(false);
//   const [progress, setProgress] = useState(0); // progress won't update without xhr, but kept for UI consistency
//   const [error, setError] = useState<string | null>(null);
//   const [successMsg, setSuccessMsg] = useState<string | null>(null);
...
//       // Assuming backend returns { data: presignedURL }
//       const putUrl = await res.json();

//       // 2. Upload video file with fetch PUT request (no progress tracking here)
//       const uploadRes = await fetch(putUrl.presignedURL, {
//         method: 'PUT',
//         headers: {
...
//       if (!uploadRes.ok) {
//         throw new Error(
//           `Upload failed for ${videoFile.name} with filename ${videoFile.name}: ${uploadRes.status} ${uploadRes.statusText}`
//         );
//       }
...
//   const [title, setTitle] = useState('');
//   const [uploading, setUploading] = useState(false);
//   const [progress, setProgress] = useState(0); // progress tracking via axios
//   const [error, setError] = useState<string | null>(null);
//   const [successMsg, setSuccessMsg] = useState<string | null>(null);
...
//       // 2. Upload video file via axios to track progress
...
//       // console.log("Uplaod successful! ETag: ", etag);
